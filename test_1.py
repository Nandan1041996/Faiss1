...
def get_most_similar_question(query_embedding):
    '''
    ...
    sql_query = f"""
    SELECT answer,embedding,embedding <=> '{lst_embedding}' AS score FROM public.que_ans_embed
    ORDER BY score desc
    limit 1;
    """
    ...
    if results:
        ...
        return 1,1,1 # Return the first similar question
...
def get_answer(similar_vector):
    ...
    ORDER BY embedding <-> '{similar_vector}' asc
    LIMIT 1;
    """
    cursor, connection = sql_connection()
    cursor.execute(sql_query)
...
if __name__ == "__main__":
    ...
    # Retrieve the most similar question from the database
    similarity_score,similar_vector,ans = get_most_similar_question(query_embedding)
